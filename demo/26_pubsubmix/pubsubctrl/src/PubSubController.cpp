...
PubSubController::PubSubController( const NERegistry::ComponentEntry & entry, ComponentThread & owner )
    ...
    , Publisher         ( static_cast<Component &>(*this) )
    , IEThreadConsumer  ( )

    , mSubscriber       ( entry.mDependencyServices[0], static_cast<Component &>(*this), 0 )
    , mConsoleThread    (static_cast<IEThreadConsumer &>(*this), entry.mRoleName + "_Thread")
{
}
