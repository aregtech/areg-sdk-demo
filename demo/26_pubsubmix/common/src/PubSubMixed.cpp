...
PubSubMixed::PubSubMixed(const NERegistry::ComponentEntry & entry, ComponentThread & owner)
    ...
    , mPubSubMixedPublisher (static_cast<Component &>(*this))
    , mControllerSubscriber (entry.mDependencyServices[0], static_cast<Component &>(*this), std::any_cast<int>(entry.getComponentData()) * 2 + 0)
    , mMixedSubscriber      (entry.mDependencyServices[1], static_cast<Component &>(*this), std::any_cast<int>(entry.getComponentData()) * 2 + 1)
{
}
